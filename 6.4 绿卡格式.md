...
实践中，child2 采用两级推导，比如泛身份为 `alt/0/0/123451234`，其下隐身份类似取 `alt/0/0/123451234/987654321/123456789`，两级比一级更安全，尤其是能防范通过暴力猜测 child2 推算验证码，进而破解泛身份与隐身份之间的关联关系（导致商家能精准实施用户画像）。
...
说明，这个 ha1 表达绿卡的自然属性，类似于会所发给会员 vip 卡，卡片封面写上卡主姓名、有效期起止时间。
...
说明，这个 ha2 表达绿卡的授权特征，好比往会所 vip 卡内安全芯片写入权限信息。
...
说明：这里 ha3 用异或运算，融合了前面 ha1 与 ha2 的值，相当于叠加绿卡的自然属性与授权特征。
